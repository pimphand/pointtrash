    ...
    <div class="sm:col-span-12  md:col-span-12 lg:col-span-8 xl:col-span-6 xl:col-start-4 ">
        <div
            ...
            <div class="flex-auto p-4 ">
                ...
                @include('components.table',['url'=>route('blogs.index'),'theads'=>[
                    '#',
                    'Judul',
                    'Tgl. Post',
                    'Thumbnail',
                            ...
                            <form id="_form_input" action="" method="post">
                                ...
                                <div class="mb-2">
                                    <label for="Mobile_Number"
                                        ...
                                        Semat</label>
                                    <textarea id="contents" name="contents" style="height: 121px;"
                                              class="form-input w-full rounded-md mt-1 border border-slate-300/60 dark:border-slate-700 dark:text-slate-300 bg-transparent px-3 py-1 focus:outline-none focus:ring-0 placeholder:text-slate-400/70 placeholder:font-normal placeholder:text-sm hover:border-slate-400 focus:border-primary-500 dark:focus:border-primary-500  dark:hover:border-slate-700"
                                              required></textarea>
                                    <div class="text-red-500 text-xs italic" id="error-contents"></div>
                                </div>
                            </form>
                        </div>
                        <div
...
@push('js')
    ...
    <script>
        ...
        function buildTableRows(data, page, per_page) {
            ...
            data.forEach((item, index) => {
                ...
                html += `<tr class="bg-white border-b border-dashed dark:bg-gray-900 dark:border-gray-700/40">
                        ...
                        <img src="{{asset('upload')}}/${item.thumbnail}" alt="" class="rounded mx-auto float-left" width="40%">
                        </td>
                        <td class="p-3 text-sm text-gray-500 whitespace-nowrap dark:text-gray-400 border dark:border-slate-700">${item.views}</td>
                        <td class="p-3 text-sm text-gray-500 whitespace-nowrap dark:text-gray-400 border dark:border-slate-700">
                            <button data-fc-type="modal" data-fc-target="_modal_form" data-id="${item.blog_id}"class="_edit_modal text-primary-500 hover:text-primary-700"><i class="fas fa-edit fa-1x"></i></button>
                            <button class="text-red-500 hover:text-danger-700 _delete" data-id="${item.blog_id}"><i class="fas fa-trash fa-1x"></i></button>
                        </td>
                    </tr>
                `;
            });
        ...
        function getData(page = 1) {
            ...
            fetchData(url, page, per_page, search, "get", function (data) {
                ...
                $('._edit_modal').on('click', function () {
                    ...
                    const itemData = itemDataArray.find(item => item.blog_id === itemId);

                    $('#title').val(itemData.title);
                    //set tinymce
                    tinymce.get('contents').setContent(itemData.content);
        ...
        $('._add_modal').on('click', function () {
            ...
            $('#_form_input input[name="_method"]').remove();
            //images
            $('#_show_thumbnail').attr('src', '{{asset('img-icon.svg')}}');
