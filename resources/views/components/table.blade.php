<table class="w-full border-collapse border dark:border-slate-700">
    <thead class="bg-slate-100 dark:bg-slate-700/20" id="thead">
    <tr>
        @php
            // Total jumlah kolom
        ...
        @foreach($theads as $index => $head)
            ...
            @else
                <th scope="col" id="{{ $head }}"
                    class="p-1 font-medium tracking-wider text-left text-gray-700 uppercase dark:text-gray-400 border dark:border-slate-700"
                    style="width: {{ $middle_columns_width }}%;">
                    <!-- Kolom tengah bagi 90% -->
                    {{ $head }}
                </th>
            @endif
        @endforeach
    ...
    <tbody id="_table-data" class="_table_data" data-url="{{$url}}"></tbody>
</table>
<div class="flex justify-between mt-4">
    <div class="self-center _show_pagination_total" id="_show_pagination_total"></div>
    <ul class="inline-flex-space-x-px list-inside my-2 py-2 _pagination" id="_pagination"></ul>
</div>
